 ...
 *
 * ⚠️  SECURITY NOTE:
 *   Hardcoding credentials here is fine for development/testing, but for
 *   production consider moving OTP sending to a backend server so credentials
 *   are never bundled in the APK.
 * ─────────────────────────────────────────────────────────────────────────────
 */
public class EmailOtpSender {
    ...
    /** Shared single-thread executor — reused across calls to avoid thread churn. */
    private static final ExecutorService EXECUTOR = Executors.newSingleThreadExecutor();

    /** Callback interface for send result. Always delivered on the main thread. */
    ...
    /**
     * Generates a 6-digit OTP, sends it to {@code recipientEmail} on a
     * background thread, then delivers the result to {@code callback} on the
     * main (UI) thread.
     *
     * @param recipientEmail  The user's registered email address.
     * @param recipientName   First name used in the email greeting.
     * @param callback        Called on the UI thread with the result.
     */
    public static void sendOtp(String recipientEmail,
        ...
        EXECUTOR.execute(() -> {
            ...
            try {
                ...
                msg.setText(buildEmailBody(recipientName, otp));
            ...
            handler.post(() -> {
                if (didSucceed) {
                    ...
                    callback.onFailure(finalError != null ? finalError : "Failed to send email.");
                }
            });
        });
    }
    ...
    /** Returns a 6-digit OTP string (100000–999999). */
